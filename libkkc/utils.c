...
static gint __lambda2_ (struct name_keysym* a, struct name_keysym* b);
...
static gint __lambda3_ (Block11Data* _data11_, struct name_keysym* a, struct name_keysym* b);
...
static struct name_keysym* kkc_key_event_utils_bsearch_keysyms (struct name_keysym* memory, glong start_offset, glong end_offset, GCompareDataFunc compare, void* compare_target, struct name_keysym* needle) {
	...
	while (TRUE) {
		...
		_tmp9_ = *_tmp8_;
		_tmp10_ = *needle;
		_tmp11_ = _tmp7_ (&_tmp9_, &_tmp10_, _tmp7__target);
		r = _tmp11_;
		_tmp12_ = r;
		if (_tmp12_ == 0) {
			struct name_keysym* _tmp13_ = NULL;
			_tmp13_ = p;
			result = _tmp13_;
			return result;
		}
		...
		if (_tmp14_ > 0) {
			...
			glong _tmp16_ = 0L;
			_tmp16_ = offset;
			start_offset = _tmp16_ + 1;
		}
		_tmp17_ = start_offset;
		_tmp18_ = end_offset;
		_tmp19_ = start_offset;
		offset = _tmp17_ + ((_tmp18_ - _tmp19_) / 2);
	}
	result = NULL;
	return result;
}
...
static gchar* kkc_key_event_utils_read_name (glong start_offset) {
	...
	const gchar* _tmp13_ = NULL;
	{
		...
		while (TRUE) {
			...
			_tmp1_ = FALSE;
			_tmp4_ = offset;
			_tmp5_ = string_get (keysym_names, _tmp4_);
			if (!(_tmp5_ != '\0')) {
				break;
			}
		}
	...
	_tmp6_ = offset;
	_tmp7_ = start_offset;
	_tmp8_ = g_malloc0 ((gsize) ((_tmp6_ - _tmp7_) + 1));
	_result_ = _tmp8_;
	_tmp9_ = _result_;
	dest = (gchar*) _tmp9_;
	_tmp10_ = start_offset;
	_tmp11_ = offset;
	_tmp12_ = start_offset;
	memcpy (dest, ((gchar*) keysym_names) + _tmp10_, (gsize) (_tmp11_ - _tmp12_));
	_tmp13_ = _result_;
	_result_ = NULL;
...
gchar* kkc_key_event_utils_keyval_name (guint keyval) {
	...
	needle = _tmp1_;
	_tmp2_ = needle;
	_tmp3_ = kkc_key_event_utils_bsearch_keysyms (keysym_to_name, (glong) 0, (glong) G_N_ELEMENTS (keysym_to_name), ___lambda2__gcompare_data_func, NULL, &_tmp2_);
	entry = _tmp3_;
	_tmp4_ = entry;
	if (_tmp4_ == NULL) {
		result = NULL;
...
static struct name_keysym* kkc_key_event_utils_find_keysym (struct name_keysym* entry, const gchar* name) {
	...
	}
	{
		...
		while (TRUE) {
			...
			_tmp8_ = FALSE;
			_tmp11_ = iter;
			if (!(_tmp11_ >= ((struct name_keysym*) name_to_keysym))) {
				break;
			}
			_tmp12_ = iter;
			_tmp13_ = (*_tmp12_).offset;
			_tmp14_ = kkc_key_event_utils_read_name ((glong) _tmp13_);
			_tmp15_ = _tmp14_;
			_tmp16_ = name;
			_tmp17_ = g_strcmp0 (_tmp15_, _tmp16_) == 0;
			_g_free0 (_tmp15_);
			if (_tmp17_) {
				struct name_keysym* _tmp18_ = NULL;
				_tmp18_ = iter;
				result = _tmp18_;
				return result;
			}
...
guint kkc_key_event_utils_keyval_from_name (const gchar* name) {
	...
	needle = _tmp2_;
	_tmp3_ = needle;
	_tmp4_ = kkc_key_event_utils_bsearch_keysyms (name_to_keysym, (glong) 0, (glong) G_N_ELEMENTS (name_to_keysym), ___lambda3__gcompare_data_func, _data11_, &_tmp3_);
	entry = _tmp4_;
	_tmp5_ = entry;
	if (_tmp5_ != NULL) {
		struct name_keysym* _tmp6_ = NULL;
