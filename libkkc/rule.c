	...
	if (_tmp3_) {
		{
			...
			while (TRUE) {
				...
				value = (JsonNode*) _tmp11_;
				{
					...
					_tmp13_ = key;
					_tmp14_ = kkc_key_event_new_from_string (_tmp13_, &_inner_error_);
					_tmp12_ = _tmp14_;
					if (_inner_error_ != NULL) {
						if (_inner_error_->domain == KKC_KEY_EVENT_FORMAT_ERROR) {
					...
					kkc_keymap_set (_tmp15_, _tmp16_, _tmp18_);
				...
				__catch6_kkc_key_event_format_error:
				{
					...
					g_warning ("rule.vala:47: can't get key event from string %s: %s", _tmp19_, _tmp21_);
					_g_error_free0 (e);
				}
				__finally6:
				if (_inner_error_ != NULL) {
...
static void kkc_keymap_map_file_class_init (KkcKeymapMapFileClass * klass) {
	...
	KKC_MAP_FILE_CLASS (klass)->uniquify = kkc_keymap_map_file_real_uniquify;
		...
		while (TRUE) {
			...
			if (_tmp14_ == JSON_NODE_ARRAY) {
				...
				if (_tmp23_) {
					...
					gchar* _tmp62_ = NULL;
					KkcRomKanaEntry _tmp63_ = {0};
					KkcRomKanaNode* _tmp64_ = NULL;
					const gchar* _tmp65_ = NULL;
					KkcRomKanaEntry _tmp66_ = {0};
					_tmp24_ = components;
					_tmp25_ = json_array_get_string_element (_tmp24_, (guint) 0);
					_tmp26_ = g_strdup (_tmp25_);
					carryover = _tmp26_;
					_tmp27_ = components;
					_tmp28_ = json_array_get_string_element (_tmp27_, (guint) 1);
					...
					if (_tmp31_ >= ((guint) 3)) {
						...
						gchar* _tmp34_ = NULL;
						_tmp32_ = components;
						_tmp33_ = json_array_get_string_element (_tmp32_, (guint) 2);
						...
						gchar* _tmp36_ = NULL;
						_tmp35_ = hiragana;
						_tmp36_ = kkc_rom_kana_utils_get_katakana (_tmp35_);
						_g_free0 (_tmp30_);
						_tmp30_ = _tmp36_;
					}
					_tmp37_ = _tmp30_;
					...
					if (_tmp40_ >= ((guint) 4)) {
						...
						_tmp41_ = components;
						_tmp42_ = json_array_get_string_element (_tmp41_, (guint) 3);
						_tmp39_ = _tmp42_;
					} else {
						_tmp39_ = "";
					}
					...
					hiragana_partial = _tmp44_;
					_tmp46_ = length;
					if (_tmp46_ >= ((guint) 5)) {
						JsonArray* _tmp47_ = NULL;
						const gchar* _tmp48_ = NULL;
						_tmp47_ = components;
						_tmp48_ = json_array_get_string_element (_tmp47_, (guint) 4);
						_tmp45_ = _tmp48_;
					} else {
						_tmp45_ = "";
					}
					...
					_tmp59_ = hiragana_partial;
					_tmp60_ = g_strdup (_tmp59_);
					_tmp61_ = katakana_partial;
					_tmp62_ = g_strdup (_tmp61_);
					_g_free0 (_tmp63_.rom);
					_tmp63_.rom = _tmp52_;
					_g_free0 (_tmp63_.carryover);
					_tmp63_.carryover = _tmp54_;
					_g_free0 (_tmp63_.hiragana);
					_tmp63_.hiragana = _tmp56_;
					_g_free0 (_tmp63_.katakana);
					_tmp63_.katakana = _tmp58_;
					_g_free0 (_tmp63_.hiragana_partial);
					_tmp63_.hiragana_partial = _tmp60_;
					_g_free0 (_tmp63_.katakana_partial);
					_tmp63_.katakana_partial = _tmp62_;
					entry = _tmp63_;
					_tmp64_ = node;
					_tmp65_ = key;
					_tmp66_ = entry;
					kkc_rom_kana_node_insert (_tmp64_, _tmp65_, &_tmp66_);
					...
					GError* _tmp67_ = NULL;
					_tmp67_ = g_error_new_literal (KKC_RULE_PARSE_ERROR, KKC_RULE_PARSE_ERROR_FAILED, "\"rom-kana\" must have two to four elements");
				...
				GError* _tmp68_ = NULL;
				_tmp68_ = g_error_new_literal (KKC_RULE_PARSE_ERROR, KKC_RULE_PARSE_ERROR_FAILED, "\"rom-kana\" member must be either an array or null");
...
KkcRomKanaMapFile* kkc_rom_kana_map_file_construct (GType object_type, KkcRuleMetadata* metadata, GError** error) {
	...
	iter = _tmp5_;
	_tmp6_ = iter;
	_tmp7_ = gee_map_iterator_first (_tmp6_);
	if (_tmp7_) {
		{
			...
			while (TRUE) {
				...
				value = (JsonNode*) _tmp15_;
				_tmp16_ = value;
				_tmp17_ = json_node_get_node_type (_tmp16_);
				if (_tmp17_ == JSON_NODE_NULL) {
					GeeMap* _tmp18_ = NULL;
					const gchar* _tmp19_ = NULL;
					_tmp18_ = parent_map;
					_tmp19_ = key;
	...
	_tmp23_ = self->root_node;
	_tmp24_ = parent_map;
	kkc_rom_kana_map_file_load_rom_kana (self, _tmp23_, _tmp24_, &_inner_error_);
	if (_inner_error_ != NULL) {
		if (_inner_error_->domain == KKC_RULE_PARSE_ERROR) {
			g_propagate_error (error, _inner_error_);
			_g_object_unref0 (iter);
...
static void kkc_rule_metadata_class_init (KkcRuleMetadataClass * klass) {
	...
	KKC_METADATA_FILE_CLASS (klass)->parse = kkc_rule_metadata_real_parse;
	...
	_tmp1_ = gee_hash_map_new (G_TYPE_STRING, (GBoxedCopyFunc) g_strdup, g_free, KKC_TYPE_RULE_METADATA, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL);
...
static gboolean kkc_rule_real_init (GInitable* base, GCancellable* cancellable, GError** error) {
	...
	KkcRuleMetadata* _tmp48_ = NULL;
	KkcKeyEventFilter* _tmp49_ = NULL;
	...
	self->priv->_keymaps_size_ = self->priv->keymaps_length1;
	{
		...
		i = _tmp6_;
		{
			...
			while (TRUE) {
				...
				_tmp10_ = i;
				_tmp11_ = enum_class;
				_tmp12_ = _tmp11_->maximum;
				if (!(_tmp10_ <= _tmp12_)) {
					break;
				}
				_tmp13_ = enum_class;
				_tmp14_ = i;
				_tmp15_ = g_enum_get_value (_tmp13_, _tmp14_);
				_tmp16_ = __g_enum_value_dup0 (_tmp15_);
				enum_value = _tmp16_;
				_tmp17_ = enum_value;
				if (_tmp17_ != NULL) {
					KkcRuleMetadata* _metadata = NULL;
					...
					KkcRuleMetadata* _tmp19_ = NULL;
					KkcRuleMetadata* _tmp20_ = NULL;
					GEnumValue* _tmp21_ = NULL;
					const gchar* _tmp22_ = NULL;
					gchar* _tmp23_ = NULL;
					gchar* _tmp24_ = NULL;
					gboolean _tmp25_ = FALSE;
					KkcKeymapMapFile* _tmp28_ = NULL;
					KkcRuleMetadata* _tmp29_ = NULL;
					GEnumValue* _tmp30_ = NULL;
					const gchar* _tmp31_ = NULL;
					KkcKeymapMapFile* _tmp32_ = NULL;
					KkcKeymapMapFile** _tmp33_ = NULL;
					gint _tmp33__length1 = 0;
					GEnumValue* _tmp34_ = NULL;
					gint _tmp35_ = 0;
					KkcKeymapMapFile* _tmp36_ = NULL;
					_tmp18_ = self->priv->_metadata;
					_tmp19_ = _g_object_ref0 (_tmp18_);
					_metadata = _tmp19_;
					_tmp20_ = _metadata;
					_tmp21_ = enum_value;
					_tmp22_ = (*_tmp21_).value_nick;
					_tmp23_ = kkc_rule_metadata_locate_map_file (_tmp20_, "keymap", _tmp22_);
					_tmp24_ = _tmp23_;
					...
					_tmp32_ = kkc_keymap_map_file_new (_tmp29_, _tmp31_, &_inner_error_);
					_tmp28_ = _tmp32_;
					if (_inner_error_ != NULL) {
						g_propagate_error (error, _inner_error_);
						_g_object_unref0 (_metadata);
					...
					_tmp33_ = self->priv->keymaps;
					_tmp33__length1 = self->priv->keymaps_length1;
					_tmp34_ = enum_value;
					_tmp35_ = (*_tmp34_).value;
					_g_object_unref0 (_tmp33_[_tmp35_]);
					_tmp33_[_tmp35_] = _tmp28_;
	...
	self->rom_kana = _tmp45_;
	_tmp48_ = _metadata;
	_tmp49_ = kkc_rule_metadata_create_key_event_filter (_tmp48_);
	_tmp50_ = _tmp49_;
	kkc_rule_set_filter (self, _tmp50_);
		...
		for (dir_it = 0; dir_it < _tmp4__length1; dir_it = dir_it + 1) {
			...
			dir = _tmp5_;
			{
				...
				while (TRUE) {
					...
					_tmp16_ = dir;
					_tmp17_ = name;
					_tmp18_ = g_build_filename (_tmp16_, _tmp17_, "metadata.json", NULL);
					metadata_filename = _tmp18_;
					...
					if (_tmp20_) {
						{
							...
							_tmp21_ = name;
							_tmp22_ = metadata_filename;
							_tmp23_ = kkc_rule_metadata_new (_tmp21_, _tmp22_, &_inner_error_);
							metadata = _tmp23_;
							if (_inner_error_ != NULL) {
							...
							_tmp26_ = metadata;
							_tmp27_ = name;
							kkc_metadata_file_set_name ((KkcMetadataFile*) _tmp26_, _tmp27_);
							_tmp28_ = rules;
							_tmp28__length1 = rules_length1;
							_tmp29_ = metadata;
							_tmp30_ = _g_object_ref0 (_tmp29_);
							_vala_array_add5 (&rules, &rules_length1, &_rules_size_, _tmp30_);
						...
						__catch9_g_error:
						{
							...
							g_warning ("rule.vala:337: can't load metadata %s: %s", _tmp31_, _tmp33_);
							_g_error_free0 (e);
						}
						__finally9:
						if (_inner_error_ != NULL) {
